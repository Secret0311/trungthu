...
// AUDIO
// AUDIO - Phát nhạc khi chạm/click bất kỳ đâu
...
function playAudioOnInteraction() {
  ...
  audio.play().then(() => {
    ...
    // Ẩn dòng chữ gợi ý (nếu có)
    if (hint) {
      hint.style.display = 'none';
    }

    // Hủy bỏ lắng nghe sự kiện sau khi đã phát nhạc thành công
    window.removeEventListener('click', playAudioOnInteraction);
    window.removeEventListener('touchstart', playAudioOnInteraction);
    window.removeEventListener('pointerdown', playAudioOnInteraction);
    window.removeEventListener('keydown', playAudioOnInteraction);
  }).catch((err) => {
    console.log("Trình duyệt chặn autoplay, chờ tương tác tiếp theo:", err);
  });
}

// Lắng nghe mọi tương tác từ chuột, chạm màn hình, phím bấm
...
window.addEventListener('pointerdown', playAudioOnInteraction, { passive: true });
